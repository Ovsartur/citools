...
Процедура ЗавершитьРаботуПриложения(Знач КодВозврата = Неопределено) Экспорт
	...
	Лог.Отладка("КодВозврата в команде ЗавершитьРаботу(КодВозврата) = " + КодВозврата);
...
Функция ЗапуститьВыполнение() Экспорт
	...
	СообщитьПолученныеПараметрыПриложенияДляОтладки(ЗначенияПараметров);

	ПроверитьОбязательныеПараметры(ЗначенияПараметров);
	
	КодВозврата = ВыполнитьКоманду(Команда, ЗначенияПараметров);
	Лог.Отладка("КодВозврата в МенеджерПриложения.ЗапуститьВыполнение() = "+КодВозврата);
	Возврат КодВозврата;	
...
Процедура ДополнитьЗначенияПараметров(Знач Команда, ЗначенияПараметров)
	...
	ОбщиеМетоды.ЗаполнитьЗначенияИзПеременныхОкружения(ЗначенияПараметров, СоответствиеПеременных);
	

	ЗначенияПараметров["UpdateProcessFileName"] = ЗначенияПараметров["TEMP_WORKDIR"] + "\1cv8.update";
	ЗначенияПараметров["FileVersions"] = ЗначенияПараметров["TEMP_WORKDIR"] + "\versions";

	Если НЕ ЗначениеЗаполнено(ЗначенияПараметров["uccode"]) Тогда
		ЗначенияПараметров["uccode"] = "123";
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЗначенияПараметров["WaitClosingScheduledJobs"]) Тогда
		ЗначенияПараметров["WaitClosingScheduledJobs"] = 1800;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(ЗначенияПараметров["WaitTimeout"]) Тогда
		ЗначенияПараметров["WaitTimeout"] = 120;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(ЗначенияПараметров["lockmessage"]) Тогда
		ЗначенияПараметров["lockmessage"] = "Выполняются регламентные работы с базой...";
	КонецЕсли; 

	Если Не ЗначениеЗаполнено(ЗначенияПараметров["lockstartat"]) Тогда
		ЗначенияПараметров["lockstartat"] = 300;
	КонецЕсли; 

КонецПроцедуры //ДополнитьЗначенияПараметров

Процедура ПроверитьОбязательныеПараметры(ЗначенияПараметров)
	
	МассивОбязательныхКЗаполнениюПараметров = Новый Массив;
	МассивОбязательныхКЗаполнениюПараметров.Добавить("TEMP_WORKDIR");
	МассивОбязательныхКЗаполнениюПараметров.Добавить("storage_release_path");

	Если ЗначенияПараметров.Получить("base_product_path") <> Неопределено 
			И НЕ ПустаяСтрока(ЗначенияПараметров.Получить("base_product_path")) Тогда
		МассивОбязательныхКЗаполнениюПараметров.Добавить("base_product_path");
	Иначе
		МассивОбязательныхКЗаполнениюПараметров.Добавить("base_product_name");
		МассивОбязательныхКЗаполнениюПараметров.Добавить("cluster_name");
	КонецЕсли;	

	Для каждого ИмяПараметра Из МассивОбязательныхКЗаполнениюПараметров Цикл		
		Ожидаем.Что(ЗначенияПараметров.Получить(ИмяПараметра) <> Неопределено 
			И НЕ ПустаяСтрока(ЗначенияПараметров.Получить(ИмяПараметра)), 
			"Ожидаем, что параметр """ + ИмяПараметра + """ заполнен, а он не заполнен!").ЭтоИстина();
	КонецЦикла;

КонецПроцедуры

Процедура СообщитьПолученныеПараметрыПриложенияДляОтладки(ЗначенияПараметров)

	Для каждого Элемент Из ЗначенияПараметров Цикл
		Лог.Отладка("Установлен следующий параметр обработки: <%1> = <%2>", Элемент.Ключ, Элемент.Значение);
	КонецЦикла;
...
Функция СоответствиеПеременныхОкруженияПараметрамКоманд()	
	...
	СоответствиеПеременных.Вставить("TEMP_WORKDIR", "TEMP_WORKDIR");
	СоответствиеПеременных.Вставить("storage_release_path", "StorageReleasePath");
	СоответствиеПеременных.Вставить("storage_release_user", "StorageReleaseAuth_USR");
	СоответствиеПеременных.Вставить("storage_release_pwd", "StorageReleaseAuth_PSW");
	СоответствиеПеременных.Вставить("cluster_name", "ClusterName");	
	СоответствиеПеременных.Вставить("cluster_user", "Cluster1CAuth_USR");
	СоответствиеПеременных.Вставить("cluster_pwd", "Cluster1CAuth_PSW");	
	СоответствиеПеременных.Вставить("base_product_name", "ProductNameBase");
	СоответствиеПеременных.Вставить("base_product_user", "BaseProductAuth_USR");
	СоответствиеПеременных.Вставить("base_product_pwd", "BaseProductAuth_PSW");	
	СоответствиеПеременных.Вставить("WaitClosingScheduledJobs", "WaitClosingScheduledJobs");
	СоответствиеПеременных.Вставить("lockstartat", "lockstartat");
	СоответствиеПеременных.Вставить("uccode", "uccode");
	СоответствиеПеременных.Вставить("lockmessage", "lockmessage");
	СоответствиеПеременных.Вставить("useordinaryapp", "UseOrdinaryApp");
...
Функция ВыполнитьКоманду(Знач ИмяКоманды, Знач ПараметрыКоманды) Экспорт
	...
	Если КодВозврата = Неопределено Тогда
		КодВозврата = РезультатыКоманд().Успех;
	КонецЕсли;
	Лог.Отладка("КодВозврата = "+КодВозврата);
	Возврат КодВозврата;
...
Функция РезультатыКоманд() Экспорт
	...
	РезультатыКоманд.Вставить("Неуспех", -1);
